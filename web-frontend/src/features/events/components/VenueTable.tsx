...
import { StatusBadge, DataTable, ConfirmDialog as DeleteDialog, EmptyState } from "@/shared/components";
...
export function VenueTable({ venues }: VenueTableProps) {
      ...
      accessor: (v: Venue) => (
        <div className="flex justify-end space-x-2">
          <Button variant="outline" size="sm" asChild>
            <Link href={`/admin/venues/edit?id=${v.id}`}>
              <Edit className="size-4 mr-2" />
              Edit
            </Link>
          </Button>
          <Button variant="outline" size="sm" asChild>
            <Link href={`/admin/venues/seat-map?id=${v.id}`}>
  ...
  return (
    <>
      <DataTable columns={columns} data={venues} keyExtractor={(v: any) => String(v.id)} />
