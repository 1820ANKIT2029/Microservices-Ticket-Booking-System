DO $$ BEGIN
    CREATE TYPE session_seat_status AS ENUM ('AVAILABLE', 'BOOKED', 'RESERVED');
EXCEPTION
...
CREATE TABLE IF NOT EXISTS session_seats (
    ...
    status session_seat_status DEFAULT 'AVAILABLE',

    CONSTRAINT uq_session_seat UNIQUE(event_session_id, seat_id),
    CONSTRAINT fk_session_seats_seat FOREIGN KEY (seat_id) REFERENCES seats (id) ON DELETE CASCADE
);